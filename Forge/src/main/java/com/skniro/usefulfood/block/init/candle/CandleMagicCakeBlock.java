...
public class CandleMagicCakeBlock extends CandleCakeBlock {
    ...
    public CandleMagicCakeBlock(Block candle) {
        super(candle, Properties.ofFullCopy(UsefulFoodBlocks.MagicCake.get()));
    ...
    public InteractionResult use(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        ...
        if (!(CandleMagicCakeBlock.isHittingCandle(hit) && player.getItemInHand(hand).isEmpty() && state.getValue(LIT))) {
            InteractionResult result = MagicCakeBlockState.eat(world, pos, UsefulFoodBlocks.MagicCake.get().defaultBlockState(), player);
            if (result.consumesAction()) CandleCakeBlock.dropResources(state, world, pos);
            return result;
        }
        CandleMagicCakeBlock.extinguish(player, state, world, pos);
